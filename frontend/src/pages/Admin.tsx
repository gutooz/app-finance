  ...
  useEffect(() => {
    ...
    getAdminStats()
      ...
      .catch(e => {
        ...
        if (status === 403) setError('Acesso negado: sua conta não tem permissão de admin.')
        else if (status === 401) navigate('/auth')
        else setError(e?.response?.data?.detail ?? 'Erro ao carregar stats')
      })
